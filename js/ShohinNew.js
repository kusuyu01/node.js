...
// 入力された文字列のバイト数をチェックする関数
function checkBytes(inputValue, itemName, bytesLimit) {
  if (inputValue > bytesLimit) {
    errorMessageList.push(
      `${itemName}は半角50文字以内、全角25文字以内で入力してください。`
    );
  }
}

// ドロップダウンリストの入力をチェックする関数
...
document.getElementById("register").addEventListener("click", function () {
  ...
  let byte = encodeURI(productName).replace(/%../g, "*").length;
  checkBytes(byte, "商品名", 50);
  ...
  let selectedValue = manufacturer.value; 
  if (selectedValue === "none") {
    errorMessageList.push("製造元を選択してください");
  }

  if (errorMessageList.length == 0) {
    window.location.href = "ShohinNewConfirm.html";
  } else {
